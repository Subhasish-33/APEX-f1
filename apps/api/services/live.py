...
class LiveService:
    ...
    async def _get_active_session(self):
        stmt = (
            select(Session, Race)
            .join(Race)
            .where(Session.state.in_([SessionState.GREEN_FLAG, SessionState.RED_FLAG, SessionState.PAUSED]))
    ...
    async def _get_degraded_leaderboard(self, start_time: float) -> ResponseEnvelope[List[Dict[str, Any]]]:
        ...
        latest_race_stmt = select(Race.race_id).where(Race.state == SessionState.COMPLETED).order_by(Race.date.desc()).limit(1)
